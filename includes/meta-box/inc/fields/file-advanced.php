...
if ( ! class_exists( 'RWMB_File_Advanced_Field' ) ) {
  class RWMB_File_Advanced_Field extends RWMB_File_Field {
    ...
    static function admin_enqueue_scripts() {
      ...
      wp_enqueue_media();
      wp_enqueue_script( 'rwmb-file-advanced', RWMB_JS_URL . 'file-advanced.js', array(
        'jquery',
        'underscore'
      ), RWMB_VER, TRUE );
    ...
    static function print_templates() {
      ...
      <script id="tmpl-rwmb-file-advanced" type="text/html">
        <# _.each( attachments, function( attachment ) { #>
          <li>
            <div class="rwmb-icon"><img
                src="<# if ( attachment.type == 'image' ){ #>{{{ attachment.sizes.thumbnail.url }}}<# } else { #>{{{ attachment.icon }}}<# } #>">
            </div>
            <div class="rwmb-info">
              <a href="{{{ attachment.url }}}" target="_blank">{{{ attachment.title }}}</a>

              <p>{{{ attachment.mime }}}</p>
              <a title="<?php echo $i18n_edit; ?>" href="{{{ attachment.editLink }}}"
                 target="_blank"><?php echo $i18n_edit; ?></a> |
              <a title="<?php echo $i18n_delete; ?>" class="rwmb-delete-file" href="#"
                 data-attachment_id="{{{ attachment.id }}}"><?php echo $i18n_delete; ?></a>
            </div>
          </li>
          <# } ); #>
      </script>
