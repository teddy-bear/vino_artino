...
<div class="event-item">
  <div id="tribe-events-event-<?php echo $event_id ?>"
  ...
  <div class="extra-info">
    <a href="<?php tribe_event_link( $post ); ?>" class="category">
      <?php
      // Output taxonomy (event category) name.
    ...
    $start_date  = tribe_get_start_date( $post, FALSE, $time_format );
    $end_date    = tribe_get_end_date( $post, FALSE, $time_format );
    echo '<time><i class="fa fa-clock-o"></i>' . $start_date . ' - ' . $end_date . '</time>';
    // Show full date
    //echo '<br>' . tribe_events_event_schedule_details();
    ?>
    <?php
    // Display resized featured image.
    $full_image_url = wp_get_attachment_url( get_post_thumbnail_id( $post->ID ) );
    $image          = aq_resize( $full_image_url, 124, '', FALSE ); //resize & crop img
    ?>
    <a href="<?php tribe_event_link( $post ); ?>"><img src="<?php echo $image; ?>"></a>
  </div>
</div>
